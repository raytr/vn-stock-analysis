...
function doPost(e) {
  try {
    ...
    if (target === -1) {
      ...
      // dòng mới phải tự mang công thức lãi/lỗ — sheet không còn kẻ sẵn lịch
      // nên không có dòng nào để kế thừa định dạng từ đó
      sheet.getRange(target, PNL_COL).setFormula(
          '=IF(OR($M$1="",$M$2="",$M$2=0,B' + target + '="",NOT(ISNUMBER(B' +
          target + '))),"",(B' + target + '-$M$1)*$M$2)');
    }
